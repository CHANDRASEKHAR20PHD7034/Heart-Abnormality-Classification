...
from Code.Feature_Selection import simulation
print("Dove Swarm Optimization was executing...")
...
class Dove:
    ...
    def move(self, function, best_pos):
        if self.behavior == Behavior.SEEKING:
            ...
            for idx_dim in range(self._dimension_size):
                # Compute velocity
                self._velocities[idx_dim] = self._velocities[idx_dim] + r1 * simulation.c1 * (
                            best_pos[idx_dim] - self._position[idx_dim])
                # Apply bounds
...
print("Dove Swarm Optimization was executed successfully...")